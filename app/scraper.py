...
class Scraper:
        ...
        while no_change_counter < 3:
            ...
            for business_snapshot in new_businesses_on_screen:
                ...
                try:
                    ...
                    previous_name = extracted_business.get("business_name")
    ...
    def _find_visible_business_card(
        ...
        if place_href:
            url_path = place_href.split('?')[0] if '?' in place_href else place_href
            matching_cards = results_panel.locator(Selectors.BUSINESS_CARD).filter(
                has=page.locator(f'a[href*="{url_path}"]')
            )
        else:
            matching_cards = results_panel.locator(Selectors.BUSINESS_CARD).filter(
    ...
    def _open_business(self, business_name: str, place_href: str | None = None, previous_name: str | None = None) -> None:
        ...
        for attempt_index, attempt_label in enumerate(attempt_labels, start=1):
            try:
                ...
                # Progressive timeout: 5s, 10s, 15s, 20s
                timeout_for_attempt = attempt_index * 5000
                if self._is_business_detail_panel_open(
    ...
    def _is_business_detail_panel_open(self, timeout: int, previous_name: str | None = None) -> bool:
        ...
        try:
            page.wait_for_function(
                ...
                ([detail_selector, previous_name]) => {
                    ...
                    if (previous_name) {
                        const currentName = heading.textContent.trim().toLowerCase();
                        const prevName = previous_name.trim().toLowerCase();
                        return currentName !== prevName;
                    }
                    return true;
                }
                """,
                arg=[Selectors.DETAIL_PANEL, previous_name],
                timeout=timeout,
            )
            return True
        except (PlaywrightError, PlaywrightTimeoutError):
